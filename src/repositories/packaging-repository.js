...
// Separate storage prevents unrelated journal operations from entering this queue.
export class PackagingRepository {
  constructor(local, remote) { this.local = local; this.remote = remote; this.running = null; }
  async list() { return (await this.local.getAll("records")).sort((a, b) => b.date.localeCompare(a.date) || b.id.localeCompare(a.id)); }
  ...
  async create(record) {
    ...
    await this.local.batch([
      ...
      { store: "operations", value: { id: record.id, requestId: makeId("packaging-request"), record: pending, error: null } }
    ]);
    return pending;
  }
  ...
  async refresh() {
    ...
    for (const item of before) if (item.syncState === "synced" && !remoteIds.has(item.id) && !pending.has(item.id)) changes.push({ store: "records", deleteKey: item.id });
