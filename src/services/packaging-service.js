...
export class PackagingService {
  ...
  async add(input, account) { const date = validDate(input.date); const item = PACKAGING_FIELDS.find(field => field.key === input.item); if (!item) throw new Error("Выберите вид упаковки"); const quantity = positive(input.quantity); const current = (await this.repository.list()).find(record => record.date === date); return this.repository.save({ ...(current || {}), id: current?.id || `packaging-day-${date}`, date, values: { ...emptyValues(), ...(current?.values || {}), [item.key]: Number(current?.values?.[item.key] || 0) + quantity }, author: account?.title || "Рабочая учётная запись" }); }
  async update(record, input, account) { return this.repository.save({ ...record, date: validDate(input.date), values: values(input), author: account?.title || record.author }); }
...
function positive(value) { const n = Number(String(value || "").replace(",", ".")); if (!Number.isFinite(n) || n <= 0) throw new Error("Количество должно быть положительным числом"); return n; }
