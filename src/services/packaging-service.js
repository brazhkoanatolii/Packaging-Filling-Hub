...
export class PackagingService {
  ...
  async add(input, account) { const date = validDate(input.date); const item = PACKAGING_FIELDS.find(field => field.key === input.item); if (!item) throw new Error("Выберите вид упаковки"); const quantity = positiveInteger(input.quantity); const author = String(input.author || account?.title || "").trim(); if (!author) throw new Error("Выберите, кто внёс данные"); const current = (await this.repository.list()).find(record => record.date === date); return this.repository.save({ ...(current || {}), id: current?.id || `packaging-day-${date}`, date, values: { ...emptyValues(), ...(current?.values || {}), [item.key]: Number(current?.values?.[item.key] || 0) + quantity }, author }); }
